...
library(dplyr)
setwd(getwd())
d_irene <- read.table("Session_20170803/Irenes_data.txt", header=T)
```
# intermediate without function

```{r}
start_time <- Sys.time()
model_outputs <- data.frame(model = character(0), aic = numeric(0), stringsAsFactors = F) # prepare output file for AIC values
for (i in 1:100){
